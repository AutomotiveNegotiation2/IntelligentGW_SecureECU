...
#ifndef	ORGAN_DHAUTO
#define	ORGAN_DHAUTO	4	/* Reference softtimer.h */
#endif
...
static void CanSrv(void)
{
	...
	if (CanTimer.TaskEn == ON)
	{
		if (CanTimer.SchudleTask >= CAN_TASK_10MS_TIME)
		{
			...
			//CANAPPINFO_PRINTF("CAN TX Task Execution...\r\n");
		}
	}
#endif
...
void CanTimer_Incr(void)
{
	...
	for(uint8_t i=0;i<STIMER_END_OF_ID-1;i++) 	SoftTimerTable[i].nCounter++;
}
...
void CAN_ACC_ON_Init(can_inst_t instance)
{
	...
	/* Setup Rx and Tx message */
	switch (instance)
	{
		case CAN_CH_3:	// CAN
			config->msgRx = (void *)(&Can3_rxmsg);
			config->msgTx = (void *)(&Can3_txmsg);
			break;
		case CAN_CH_1:	// CAN-FD
			config->msgRx = (void *)(&Can1_rxmsg);
			config->msgTx = (void *)(&Can1_txmsg);
			break;
	}

	/* Initialize stimer_state and msgUpdate */
	memset(((CAN_MSG_FUNC_ptr *)config->msgTx)->stimer_state, CAN_TX_PERI_FUNC_STOP, ((CAN_MSG_FUNC_ptr *)config->msgTx)->NoOfMsg);
	memset(((CAN_MSG_FUNC_ptr *)config->msgTx)->msgUpdate, MSG_UPDATE_COMPLETE, ((CAN_MSG_FUNC_ptr *)config->msgTx)->NoOfMsg);
}
...
static void CanTaskStart(void)
{
	...
	CanTimer.TaskEn = ON;
	SetSoftTimer(STIMER_CAN_TASK_START, CAN_TASK_10MS_TIME, CanTxTask);
}
...
static void CanTaskStop(void)
{
	CanTimer.SchudleTask = 0;
	CanTimer.TaskEn = OFF;
	KillSoftTimer(STIMER_CAN_TASK_START);
}
