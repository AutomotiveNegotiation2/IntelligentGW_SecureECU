...
//	led_op        led_pos    dev_id         mem_addr   num_words  data  wtime
	OP_LED_TURN_RIGHT, PWR_WH   , NULL          , NULL        , 0, OFF   , 0,
	OP_LED_TURN_RIGHT, PWR_YEL  , NULL          , NULL        , 0, ON    , 40,
	...
	OP_LED_TURN_RIGHT, PWR_WH   , NULL          , NULL        , 0, ON   , 0,
	OP_LED_TURN_RIGHT, PWR_YEL  , NULL          , NULL        , 0, OFF  , 40, 
};

static elmos_drv_config_t turn_left_seq[] = {
//	led_op		  led_pos	 dev_id 		mem_addr   num_words  data	wtime
	OP_LED_TURN_LEFT, PWR_WH	, NULL			, NULL		  , 0, OFF	 , 0,
	OP_LED_TURN_LEFT, PWR_YEL	, NULL			, NULL		  , 0, ON	 , 40,
	...
	OP_LED_TURN_LEFT, PWR_WH   , NULL          , NULL        , 0, ON   , 0,
	OP_LED_TURN_LEFT, PWR_YEL  , NULL          , NULL        , 0, OFF  , 40, 
};
...
//	led_op		  led_pos	 dev_id 		mem_addr   num_words  data	wtime
	OP_LED_EMERGENCY, PWR_WH	, NULL			, NULL		  , 0, OFF	 , 0,
	OP_LED_EMERGENCY, PWR_YEL	, NULL			, NULL		  , 0, ON	 , 40,
	...
	OP_LED_EMERGENCY, PWR_WH	, NULL			, NULL		  , 0, ON	, 0,
	OP_LED_EMERGENCY, PWR_YEL	, NULL			, NULL		  , 0, OFF	, 40, 
};

elmos_drv_config_t welcome_seq[] = {
//	led_op		  led_pos	 dev_id 		mem_addr   num_words  data	wtime
	OP_LED_WELCOME, PWR_WH	, NULL			, NULL		  , 0, ON	 , 0,
	OP_LED_WELCOME, PWR_YEL	, NULL			, NULL		  , 0, OFF	 , 40,
	...
	OP_LED_WELCOME, PWR_WH	, NULL			, NULL		  , 0, OFF	 , 0,
	OP_LED_WELCOME, PWR_YEL	, NULL			, NULL		  , 0, ON	 , 40,
};
...
#endif
uint8_t right_write_data(uint8_t mem_addr, uint8_t dev_addr, uint8_t num_words, uint8_t * write_data);
uint8_t left_write_data(uint8_t mem_addr, uint8_t dev_addr, uint8_t num_words, uint8_t * write_data);
...
void lightingGrill_OpStart(uint16_t led_op, uint16_t repTimes)
{
	...
	//LIGHTING_GRILL_WHITE_ON;
	//LIGHTING_GRILL_YELLOW_OFF;
	SetSoftTimer(STIMER_IOCTRL_LIGHTING_GRILL_LED_OP, 50, lightingGrill_OpSeq);
...
#else
		SDK_DelayAtLeastUs(drv_cfg->wtime * 1000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
#endif
		led_ctrl.seq_cur++;
		//goto LOOP1;
	}
...
#else
			SDK_DelayAtLeastUs(40 * 1000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
			goto LOOP1;
#endif
		}
...
static uint8_t header_data[5] = {0x00, 0x55};
...
#endif
{
...
#else
    uint8_t crc = 0;
    uint8_t error_data[20] = {0x00};
    uint8_t i = 0;
    uint8_t data_len = 0;
    uint8_t num_words_tmp = (num_words - 1);
	...
	write_data[1] = (data & 0x300) >> 8;

    //3byte_header
    header_data[2] = mem_addr;
    header_data[3] = (dev_addr & 0x1f) | (WRITE_BIT << 5) | ((num_words_tmp & 0x03) << 6);
    
    crc = crc_calc(CRC_6_INIT, calc_rev_poly(CRC_6), header_data[2]);
    crc = crc_calc(crc, calc_rev_poly(CRC_6), header_data[3]);
    crc = crc_calc(crc, calc_rev_poly(CRC_6), ((num_words_tmp & 0x0c) >> 2));
    header_data[4] = (crc << 2) | ((num_words_tmp & 0x0c) >> 2);

    LPUART_SetBaudRate(lpuart, 399881, CLOCK_GetRootClockFreq(root));
    LPUART_WriteBlocking(lpuart, &header_data[0], 1);
    LPUART_SetBaudRate(lpuart, 500000, CLOCK_GetRootClockFreq(root));

    LPUART_WriteBlocking(lpuart, &header_data[1], 4);

    SDK_DelayAtLeastUs(200, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);

    LPUART_ReadBlocking(lpuart, error_data, 6);
    if(((error_data[5] & 0x0f) != 0x00) || ((error_data[5] & 0x07) != 0x00) || ((error_data[5] & 0x03) != 0x00)){
        PRINTF("header error data : 0x%x\r\n", error_data[5]);
        return 0;
    }
    ...
    if(num_words < 11){
        crc = crc_calc(CRC_8_INIT, calc_rev_poly(CRC_8_L_11), write_data[0]);
        for(i = 1; i < data_len; i++){
            crc = crc_calc(crc, calc_rev_poly(CRC_8_L_11), write_data[i]);
        }
        write_data[i] = crc;
    }
    else{
        crc = crc_calc(CRC_8_INIT, calc_rev_poly(CRC_8_H_11), write_data[0]);
        for(i = 1; i < data_len; i++){
            crc = crc_calc(crc, calc_rev_poly(CRC_8_H_11), write_data[i]);
        }
        write_data[i] = crc;
    }
    LPUART_WriteBlocking(lpuart, write_data, (data_len + 1));
#if 0
    SDK_DelayAtLeastUs(200, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
...
uint8_t right_write_data(uint8_t mem_addr, uint8_t dev_addr, uint8_t num_words, uint8_t * write_data)
{
...
#else
	buf[0] = BUS_BREAK_BYTE;
	LPUART_SetBaudRate(LPUART11, 399881, CLOCK_GetRootClockFreq(kCLOCK_Root_Lpuart11));
	LPUART_WriteBlocking(LPUART11, &buf[0], 1);
	...
	//LPUART_SetBaudRate(LPUART11, 399881, CLOCK_GetRootClockFreq(kCLOCK_Root_Lpuart11));
	//LPUART_WriteBlocking(LPUART11, &buf[0], 1);
	LPUART_SetBaudRate(LPUART11, 500000, CLOCK_GetRootClockFreq(kCLOCK_Root_Lpuart11));
	
	LPUART_WriteBlocking(LPUART11, &buf[0], 4);
	
	SDK_DelayAtLeastUs(100, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
	
	LPUART_ReadBlocking(LPUART11, error_data, 6);
	if(((error_data[5] & 0x0f) != 0x00) || ((error_data[5] & 0x07) != 0x00) || ((error_data[5] & 0x03) != 0x00)){
		PRINTF("header error data : 0x%x\r\n", error_data[5]);
		return 0;
	}
	...
	LPUART_WriteBlocking(LPUART11, write_data, (data_len + 1));
#if 0
		SDK_DelayAtLeastUs(200, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
...
uint8_t left_write_data(uint8_t mem_addr, uint8_t dev_addr, uint8_t num_words, uint8_t * write_data)
{
...
#else
	buf[0] = BUS_BREAK_BYTE;
	LPUART_SetBaudRate(LPUART10, 399881, CLOCK_GetRootClockFreq(kCLOCK_Root_Lpuart10));
	LPUART_WriteBlocking(LPUART10, &buf[0], 1);
	...
	//LPUART_SetBaudRate(LPUART10, 399881, CLOCK_GetRootClockFreq(kCLOCK_Root_Lpuart10));
	//LPUART_WriteBlocking(LPUART10, &buf[0], 1);
	LPUART_SetBaudRate(LPUART10, 500000, CLOCK_GetRootClockFreq(kCLOCK_Root_Lpuart10));
		
	LPUART_WriteBlocking(LPUART10, &buf[0], 4);

	SDK_DelayAtLeastUs(100, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
	
	LPUART_ReadBlocking(LPUART10, error_data, 6);
	if(((error_data[5] & 0x0f) != 0x00) || ((error_data[5] & 0x07) != 0x00) || ((error_data[5] & 0x03) != 0x00)){
		PRINTF("header error data : 0x%x\r\n", error_data[5]);
		return 0;
	}
	...
	LPUART_WriteBlocking(LPUART10, write_data, (data_len + 1));
#if 0
	SDK_DelayAtLeastUs(200, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
	
	memcpy(error_data, data_clear, sizeof(error_data));
	LPUART_ReadBlocking(LPUART10, error_data, (data_len+2));
	if(((error_data[data_len+2] & 0x0f) != 0x00) || ((error_data[data_len+2] & 0x07) != 0x00) || ((error_data[data_len+2] & 0x03) != 0x00)){
		PRINTF("data error data : 0x%x\r\n", error_data[data_len+2]);
		return 0;
	}
...
#if 0
uint8_t data[20]  = {0};
uint8_t data_clear[20] = {0, };

void turn_right_sig(void){
    uint16_t data_num;

	LIGHTING_GRILL_WHITE_OFF;
	LIGHTING_GRILL_YELLOW_ON;
	
    memcpy(data, data_clear, sizeof(data));

    data_num = LED_H;
    data[0] = (data_num & 0xff);
    data[1] = (data_num & 0x300) >> 8;

    led_disp_data(LED_RIGHT, BUS_PULSE_0, ELMOS_DEV_ID_2, 1, data_num);
    SDK_DelayAtLeastUs(40000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_8, ELMOS_DEV_ID_2, 1, data_num);
    SDK_DelayAtLeastUs(40000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_0, ELMOS_DEV_ID_3, 1, data_num);
    SDK_DelayAtLeastUs(40000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_8, ELMOS_DEV_ID_3, 1, data_num);
    SDK_DelayAtLeastUs(40000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_0, ELMOS_DEV_ID_4, 1, data_num);
    SDK_DelayAtLeastUs(40000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_8, ELMOS_DEV_ID_4, 1, data_num);
    SDK_DelayAtLeastUs(40000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_0, ELMOS_DEV_ID_5, 1, data_num);
    SDK_DelayAtLeastUs(40000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_8, ELMOS_DEV_ID_5, 1, data_num);
    SDK_DelayAtLeastUs(40000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_0, ELMOS_DEV_ID_6, 1, data_num);
    SDK_DelayAtLeastUs(40000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
...
void remove_sig(void){
    ...
    led_disp_data(LED_RIGHT, BUS_PULSE_0, ELMOS_DEV_ID_6, 1, data_num);
    led_disp_data(LED_LEFT, BUS_PULSE_0, ELMOS_DEV_ID_6, 1, data_num);
    SDK_DelayAtLeastUs(500, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_0, 0, 1, data_num);
    led_disp_data(LED_LEFT, BUS_PULSE_0, 0, 1, data_num);
    SDK_DelayAtLeastUs(500, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_2, ELMOS_DEV_ID_1, 1, data_num);
    led_disp_data(LED_LEFT, BUS_PULSE_2, ELMOS_DEV_ID_1, 1, data_num);
    SDK_DelayAtLeastUs(500, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_4, ELMOS_DEV_ID_1, 1, data_num);
    led_disp_data(LED_LEFT, BUS_PULSE_4, ELMOS_DEV_ID_1, 1, data_num);
    SDK_DelayAtLeastUs(500, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_6, ELMOS_DEV_ID_1, 1, data_num);
    led_disp_data(LED_LEFT, BUS_PULSE_6, ELMOS_DEV_ID_1, 1, data_num);
    SDK_DelayAtLeastUs(500, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_8, 0, 1, data_num);
    led_disp_data(LED_LEFT, BUS_PULSE_8, 0, 1, data_num);
    SDK_DelayAtLeastUs(500, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    led_disp_data(LED_RIGHT, BUS_PULSE_10, ELMOS_DEV_ID_1, 1, data_num);
    led_disp_data(LED_LEFT, BUS_PULSE_10, ELMOS_DEV_ID_1, 1, data_num);
    SDK_DelayAtLeastUs(500, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
}
#endif
