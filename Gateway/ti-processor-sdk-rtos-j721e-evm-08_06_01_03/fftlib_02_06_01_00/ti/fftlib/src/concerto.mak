...
# enable global FFTLIB check params if CHECKPARAMS is selected
ifeq ($(CHECKPARAMS),1)
FFTLIB_DEFS += FFTLIB_CHECK_PARAMS
endif

# enable FFTLIB_MESG debug messages whenever compiled in debug mode
ifeq ($(TARGET_BUILD),debug)
FFTLIB_DEFS += FFTLIB_MESSAGES
endif

ifneq ($(PERFORMANCE_TEST),)
FFTLIB_DEFS += PERFORMANCE_TEST=$(PERFORMANCE_TEST)
endif

ifneq ($(COMPILER_TEST),)
FFTLIB_DEFS += COMPILER_TEST=$(COMPILER_TEST)
endif

FFTLIB_DEFS += EXTENDED_TEST_CASE=$(EXTENDED_TEST_CASE)
ifneq ($(EXTENDED_TEST_CASE), -1)
FFTLIB_DEFS += TEST_CASE=$(EXTENDED_TEST_CASE)
else
FFTLIB_DEFS += TEST_CASE=$(TEST_CASE)
endif

FFTLIB_DEFS += TEST_CATEGORY=$(TEST_CATEGORY)
...
FFTLIB_DEFS += ALL_TEST_CASES
endif
endif
endif

ifeq ($(NO_PRINTF), 1)
FFTLIB_DEFS += NO_PRINTF
endif

ifneq ($(FLAT_MEM),)
FFTLIB_DEFS += FLAT_MEM
endif

ifneq ($(FFTLIB_NUMBER_RANDOM_DIM_TESTS),)
FFTLIB_DEFS += FFTLIB_NUMBER_RANDOM_DIM_TESTS=$(FFTLIB_NUMBER_RANDOM_DIM_TESTS)
endif

ifneq ($(FFTLIB_DEBUGPRINT),)
FFTLIB_DEFS += FFTLIB_DEBUGPRINT=$(FFTLIB_DEBUGPRINT)
endif

ifneq ($(FFTLIB_SKIP_CI),)
FFTLIB_DEFS += FFTLIB_SKIP_CI=$(FFTLIB_SKIP_CI)
endif

ifneq ($(FFTLIB_SKIP_CN),)
FFTLIB_DEFS += FFTLIB_SKIP_CN=$(FFTLIB_SKIP_CN)
endif

ifneq ($(FFTLIB_SKIP_EXEC),)
FFTLIB_DEFS += FFTLIB_SKIP_CN=$(FFTLIB_SKIP_EXEC)
FFTLIB_DEFS += FFTLIB_SKIP_CI=$(FFTLIB_SKIP_EXEC)
endif

ifneq ($(FIXED_SEED),)
FFTLIB_DEFS += FFTLIB_FIXED_SEED=$(FIXED_SEED)
endif
...
ifneq ($(FFTLIB_COMPILER_VERSION),)
FFTLIB_DEFS += FFTLIB_COMPILER_VERSION=$(FFTLIB_COMPILER_VERSION)
else
FFTLIB_DEFS += FFTLIB_COMPILER_VERSION=0
endif

SUBMODULE_LIST :=

EXCLUSION_LIST := . common
SUBMODULE_LIST += fft_c7xmma linalg_c7xmma fft_c7x

# things that shouldn't get built because there are known issues with this code
# EXCLUSION_LIST += linalg_c7xmma/FFTLIB_LINALG_matrixMatrixMultiply_ixX_ixX_oxX
# EXCLUSION_LIST += linalg_c7xmma/FFTLIB_LINALG_matrixMatrixMultiplyAccumulate_ixX_ixX_ixX_oxX
# EXCLUSION_LIST += linalg_c7xmma/FFTLIB_LINALG_matrixTranspose_ixX_oxX
# EXCLUSION_LIST += linalg_c7xmma/FFTLIB_LINALG_pointwiseMatrixMatrixMultiply_ixX_ixX_oxX
# EXCLUSION_LIST += fft_c7xmma/FFTLIB_FFT_dftLarge_ixX_cxX_oxX
# EXCLUSION_LIST += fft_c7xmma/FFTLIB_FFT_dftSmall_ixX_cxX_oxX
# EXCLUSION_LIST += fft_c7xmma/FFTLIB_FFT_fft_ixX_cxX_oxX
# EXCLUSION_LIST += fft_c7xmma/FFTLIB_FFT_highRadixDecompositions_ixX_cxX_oxX
# EXCLUSION_LIST += cnn_c7xmma/FFTLIB_CNN_convolve_row_ixX_ixX_oxX
# EXCLUSION_LIST += cnn_c7xmma/FFTLIB_CNN_deconvolve_row_ixX_ixX_oxX
# EXCLUSION_LIST += cnn_c7xmma/FFTLIB_CNN_fullyConnected_ixX_ixX_oxX
# EXCLUSION_LIST += cnn_c7xmma/FFTLIB_CNN_add_ixX_ixX_oxX
# EXCLUSION_LIST += dsp_c7xmma/FFTLIB_DSP_fir_ixX_ixX_oxX
# EXCLUSION_LIST += dsp_c7xmma/FFTLIB_DSP_firSmall_ixX_ixX_oxX
ifeq ($(TARGET_CPU),C7100)
EXCLUSION_LIST += cnn_c7xmma/FFTLIB_CNN_convolveBias_row_ixX_ixX_oxX
EXCLUSION_LIST += cnn_c7xmma/FFTLIB_CNN_fullyConnectedBias_ixX_ixX_oxX
EXCLUSION_LIST += cnn_c7xmma/FFTLIB_CNN_convolve_col_smallNo_legacy_ixX_ixX_oxX
EXCLUSION_LIST += cnn_c7xmma/FFTLIB_CNN_convolve_col_smallNo_highPrecision
EXCLUSION_LIST += cnn_c7xmma/FFTLIB_CNN_pixelShuffleBias_row_ixX_ixX_oxX
...
ifneq ($(TARGET_SCPU),)
EXCLUDE_FILE := common/c7100/FFTLIB_UTIL_mma_bist.asm
ASSEMBLY := $(filter-out $(EXCLUDE_FILE), $(ASSEMBLY))
#ifdef SystemRoot
#CSOURCES := common/C71/FFTLIB_configurations.c
#endif
# Hack Alert:  SystemRoot is only defined in Windows build; not sure why this is necessary
ifdef SystemRoot
CSOURCES := common/$(TARGETPARENTFOLDER)/FFTLIB_configurations.c
CSOURCES += common/$(TARGETFOLDER)/FFTLIB_configurations.c
endif
endif
DEFS :=
ifneq ($(LINK_L2SRAM),)
...
ifneq ($(TARGET_SCPU),)
EXCLUDE_FILE := test/c71/FFTLIB_TEST_c7xecr.asm
EXCLUDE_FILE += test/c71/FFTLIB_TEST_c7xl2cc.asm
ASSEMBLY := $(filter-out $(EXCLUDE_FILE), $(ASSEMBLY))
# ifdef SystemRoot
# CSOURCES := common/C71/FFTLIB_configurations.c
# endif
...
#List all sub-categories of sources for building libraries (fftlib, fftlib_cn, fftlib_d)
...
#DEFS :=
#ifeq ($(TARGET_BUILD),debug)
# DEFS += FFTLIB_CHECK_PARAMS
#DEFS += FFTLIB_MESSAGES
#endif
#DEFS += $(FFTLIB_DEFS)
include $(FINALE)
...
TARGETTYPE = exe
STATIC_LIBS :=
STATIC_LIBS += fftlib_$(TARGET_CPU) fftlib_cn_$(TARGET_CPU)
# TEMPORARY DEFINITION TO BUILD OPTIMIZED LIBRARY WHEN NOT IN HOST EMULATION MODE
# STATIC_LIBS += fftlib_cn_$(TARGET_CPU)
ifeq ($$(TARGET_SCPU),)
STATIC_LIBS += fftlib_$(TARGET_CPU)
endif
STATIC_LIBS += fftlib_cn_$(TARGET_CPU)
ifeq ($(TARGET_SCPU), RESERVED)
SYS_STATIC_LIBS += RESERVED-host-emulation
...
ifneq (,$(filter $(TARGET_SCPU),C7100 RESERVED RESERVED RESERVED RESERVED))
SYS_STATIC_LIBS += fftlib_$(TARGET_CPU) fftlib_cn_$(TARGET_CPU) common_$(TARGET_CPU) test_$(SOC)
else
SYS_STATIC_LIBS += fftlib_$(TARGET_CPU).lib fftlib_cn_$(TARGET_CPU).lib common_$(TARGET_CPU).lib test_$(SOC).lib
endif
# ifeq ($$(TARGET_SCPU),)
# STATIC_LIBS += fftlib_$(TARGET_CPU)
# endif
