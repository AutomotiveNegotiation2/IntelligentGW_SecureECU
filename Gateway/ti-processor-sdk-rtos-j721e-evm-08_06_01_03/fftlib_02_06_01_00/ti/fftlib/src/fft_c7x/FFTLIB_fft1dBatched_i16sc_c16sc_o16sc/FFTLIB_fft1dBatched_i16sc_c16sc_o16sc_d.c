...
uint8_t FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_pBlock
...
int FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_d (uint32_t *pProfile,
                                             uint8_t   LevelOfFeedback)
{
   ...
   FILE *fpOutputCSV =
       fopen ("FFTLIB_fft1dBatched_i16sc_c16sc_o16sc.csv", "w+");
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      if (pX && pY && pW && pXCn && pYCn && pShift) {
         ...
         TI_fillBuffer (prm[tpi].testPattern, 0, pX, prm[tpi].staticIn0,
                        bufWidth, bufHeight, bufWidth * sizeof (int16_t),
         ...
         TI_fillBuffer (STATIC, 0, pXCn, pX, bufWidth, bufHeight,
                        bufWidth * sizeof (int16_t), sizeof (int16_t),
                        testPatternString);
         ...
         status_opt = FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_init (
             (int16_t *) pX, &bufParamsData, (int16_t *) pW, &bufParamsTw,
             (int16_t *) pY, &bufParamsData, (uint32_t *) pShift,
             &bufParamsShift, numPoints, numChannels,
...
not remove
         ...
         status_nat = FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_cn (
             (int16_t *) pXCn, &bufParamsData, (int16_t *) pW, &bufParamsTw,
             (int16_t *) pYCn, &bufParamsData, (uint32_t *) pShift,
             &bufParamsShift, numPoints, numChannels);
         TI_profile_stop ();

         status_nat_vs_opt =
             TI_compare_mem ((void *) pY, (void *) pYCn,
