                                               ...
                                               uint8_t   LevelOfFeedback)
{
   ...
   uint32_t num_pts    = 0;
   /* Test Parameters */
   pixelShuffle_row_ixX_ixX_oxX_testParams_t *prm;

   pixelShuffle_row_ixX_ixX_oxX_getTestParams (&prm, &test_cases);
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      if (src0 && src1 && dst) {
         ...
         MMALIB_asm (" MARK 0");
         status_init = MMALIB_CNN_pixelShuffle_row_ixX_ixX_oxX_init (
         ...
         for (NCount = 0; NCount < totalN; NCount += subN) {
            ...
            for (MCount = 0; MCount < numOutChannels; MCount += subMChannels) {
               ...
               MMALIB_asm (" MARK 2");
               status_opt = MMALIB_CNN_pixelShuffle_row_ixX_ixX_oxX_exec (
         ...
         for (NCount = 0; NCount < totalN; NCount += subN) {
            ...
            for (MCount = 0; MCount < numOutChannels; MCount += subMChannels) {
               ...
               MMALIB_asm (" MARK 4");
               status_nat = MMALIB_CNN_pixelShuffle_row_ixX_ixX_oxX_exec (
