...
#include "MMALIB_FFT_dftLarge_ixX_cxX_oxX_idat.h"

__attribute__ ((section (".ddrData"), aligned (64))) int8_t ddrBuffer[2048 * 1024];
int16_t volatile volatileSum = 0;  // use volatile to keep compiler from removing this operation

int MMALIB_FFT_dftLarge_ixX_cxX_oxX_d(
   uint32_t *pProfile,
   uint8_t LevelOfFeedback)
{
   ...
   MMALIB_FFT_dftLarge_ixX_cxX_oxX_InitArgs    kerInitArgs;

   int32_t handleSize = MMALIB_FFT_dftLarge_ixX_cxX_oxX_getHandleSize(&kerInitArgs);
   MMALIB_kernelHandle handle = malloc(handleSize);

   TI_profile_init("MMALIB_FFT_dftLarge_ixX_cxX_oxX");
   ...
   for (tpi=0; tpi < test_cases; tpi++) {
	 ...
	 for (k = 0; k < MMALIB_FFT_DFTLARGE_IXX_CXX_OXX_NUMSHIFTS; k++) {
	 ...
	 MMALIB_FFT_dftLarge_ixX_cxX_oxX_getSizes(&kerInitArgs,
						  &bufParamsX,
						  &bufParamsW,
						  &bufParamsY);
...
//	 void *pYCn = (void *) malloc(bufParamsX.dim_x*MMALIB_sizeof(currPrm.dataType));
	 ...
	 if( pX && pW && pY && pYCn ) {
	    ...
	    MMALIB_FFT_dftLarge_ixX_cxX_oxX_twGen(&kerInitArgs,
						  pW,
						  &bufParamsW);

	    status_init = MMALIB_FFT_dftLarge_ixX_cxX_oxX_init_checkParams(handle,
									   &bufParamsX,
									   &bufParamsW,
									   &bufParamsY,
									   &kerInitArgs);
	    ...
	    if (status_init == MMALIB_SUCCESS) {
	       ...
	       status_init = MMALIB_FFT_dftLarge_ixX_cxX_oxX_init(handle,
								  &bufParamsX,
								  &bufParamsW,
								  &bufParamsY,
								  &kerInitArgs);
	    ...
	    status_opt = MMALIB_FFT_dftLarge_ixX_cxX_oxX_exec_checkParams(handle,
									  pX,
									  pW,
									  pY);
	    ...
	    if(status_opt == MMALIB_SUCCESS){
	       ...
	       MMALIB_asm(" MARK 2");
	       status_opt = MMALIB_FFT_dftLarge_ixX_cxX_oxX_exec(handle,
								 pX,
								 pW,
								 pY);
	       MMALIB_asm(" MARK 3");
	    ...
	    MMALIB_FFT_dftLarge_ixX_cxX_oxX_init(handle,
						 &bufParamsX,
						 &bufParamsW,
						 &bufParamsY,
						 &kerInitArgs);
	    ...
	    MMALIB_asm(" MARK 8");
	    status_nat = MMALIB_FFT_dftLarge_ixX_cxX_oxX_exec(handle,
							      pX,
							      pW,
							      pYCn);
	    MMALIB_asm(" MARK 9");
	    ...
	    MMALIB_FFT_dftLarge_ixX_cxX_oxX_perfEst(handle,
						    &bufParamsX,
						    &bufParamsY,
						    &bufParamsW,
						    currPrm.fftSize,
...
int test_main(uint32_t *pProfile) {
   ...
   if( TI_cache_init()) {
      TI_memError("MMALIB_FFT_dftLarge_ixX_cxX_oxX");
      return 1;
   } else
#else
...
#endif
   {
      return MMALIB_FFT_dftLarge_ixX_cxX_oxX_d(&pProfile[0],
					       0);
   }
}
...
/* ======================================================================== */
/*  End of file:  MMALIB_FFT_dftLarge_ixX_cxX_oxX_d.c                          */
/* ======================================================================== */
