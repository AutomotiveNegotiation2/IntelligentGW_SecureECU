...
int MMALIB_DSP_fir_ixX_ixX_oxX_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   fir_ixX_ixX_oxX_testParams_t   *prm;
   fir_ixX_ixX_oxX_testParams_t   currPrm;
   fir_ixX_ixX_oxX_getTestParams(&prm, &test_cases);
   ...
   for (tpi=0; tpi < test_cases; tpi++) {
	 ...
	 if( pIn && pOut && pFilter && pOutCn ) {
	    ...
	    status_opt = MMALIB_DSP_fir_ixX_ixX_oxX_exec_checkParams(handle,
								     pIn,
								     pFilter,
								     pOut);
