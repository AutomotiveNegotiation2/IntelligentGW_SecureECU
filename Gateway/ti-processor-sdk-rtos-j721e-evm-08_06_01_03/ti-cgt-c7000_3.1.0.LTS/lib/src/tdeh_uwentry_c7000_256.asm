...
||      MVK64   .L1   TI_UNWIND_RET, A3 ; Set return pointer in case of failure
        ...
        ;
        ; The frame's 16 free byte block is used to store SP and PC, and 160
        ...
        ; Begin storing SOEs:
        STD     .D1   A0,*SP++(-160)    ; Store caller's SP to register buffer
...
||      MVC     .S1   A3, RP            ; RP is now TI_UNWIND_RET
        ...
        VST4D  .D2   VB14,*SP[_Unwind_Reg_Id._UR_VB14_Offset + 2]
||      STD    .D1   A13,*SP[_Unwind_Reg_Id._UR_A13 + 2]

        VST4D  .D2   VB15,*SP[_Unwind_Reg_Id._UR_VB15 + 2]
...
TI_UNWIND_RET:
       ...
       MVC     .S1   A0,RP
||     ADDD    .D1   SP,160,SP                      ; Restore stack
...
__TI_Install_CoreRegs: .asmfunc
        ...
        VLD4D  .D1    *A4[_Unwind_Reg_Id._UR_VB14_Offset], VB14
||      LDD    .D2    *A4[_Unwind_Reg_Id._UR_PC_Offset],   A0

        VLD4D  .D1    *A4[_Unwind_Reg_Id._UR_VB15],        VB15
...
__cxa_end_cleanup: .asmfunc
        ...
        ; 136 = 56 (scalar SOEs) + 64 (vector SOEs) + 16 (free byte block)
        STD    .D1   A9,*SP++(-136)
||      STD    .D2X  A8,*SP(8)
