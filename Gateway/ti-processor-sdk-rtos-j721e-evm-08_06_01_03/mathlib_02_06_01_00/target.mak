...
# This appears in both the if and else clause; should be moved to appropriate location but I don't know what that is
    ...
    TARGET_CPU ?= C7100
    ifneq ($(TARGET_CPU),$(filter $(TARGET_CPU),C7100 RESERVED RESERVED RESERVED RESERVED))
...
help:
	...
	@echo $(q)                           Available options: C7100, RESERVED, RESERVED, x86_64 (Linux). [Default=C7100]$(q)
	@echo $(q)   TARGET_SCPU        - When TARGET_CPU==x86_64, this is the simulated target CPU to build.$(q)
	@echo $(q)                           Available options: C7100, RESERVED, RESERVED. [Default=C7100]$(q)
	...
	@echo $(q)                           Available options: J7ES, J7AEP, J7AHP, RESERVED. [Default=J7ES]$(q)
	...
	@echo $(q)   NO_PRINTF          - Disable all printf statements in MATHLIB builds. Expected to be used when TEST_ENV is QT or RTL.$(q)
	...
	@echo $(q)   FIXED_SEED         - Seed value to initialize the random number generator used for test generation in MATHLIB.$(q)
	...
	@echo $(q)   AUTOTEST           - Flag to link each unit test against the mathlib and mathlib_cn libraries instead of $(q)
	...
	@echo $(q)   $(MT)$(q)
	@echo $(q)   $(MT) -j8$(q)
	@echo $(q)   $(MT) -j8 mathlib$(q)
	@echo $(q)   $(MT) -j8 linalg_c7xmma/MATHLIB_LINALG_matrixMatrixMultiply_ixX_ixX_oxX$(q)
	@echo $(q)   $(MT) -j12 cnn_c7xmma/MATHLIB_CNN_convolve_col_smallNo_ixX_ixX_oxX TEST_CATEGORY=200 TARGET_CPU=x86_64 TARGET_SCPU=C7100 TARGET_PLATFORM=PC TARGET_BUILD=release$(q)
	@echo $(q)   $(MT) -j8 cnn_c7xmma/MATHLIB_CNN_convolve_row_ixX_ixX_oxX TEST_ENV=QT PERFORMANCE_TEST=1 NO_PRINTF=1 TEST_CASE=195$(q)
	$(NEWLINE)
