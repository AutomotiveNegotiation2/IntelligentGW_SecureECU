...
int32_t DSPLIB_cascadeBiquad_getHandleSize(
...
DSPLIB_STATUS DSPLIB_cascadeBiquad_init_checkParams(
...
DSPLIB_STATUS DSPLIB_cascadeBiquad_exec_checkParams(
  DSPLIB_kernelHandle handle,
...
inline dataType fast_recip(dataType x)
{
...
dataType DSPLIB_cascadeBiquad_store_coefficients(uint32_t numStages, void *restrict pFilterCoeff)
{
   ...
   for (uint32_t coeffCount = 0; coeffCount < numStages*5; coeffCount = coeffCount + 5) {
      ...
      float r = fast_recip<dataType>(coeff0);
...
DSPLIB_STATUS DSPLIB_cascadeBiquad_init(
              DSPLIB_kernelHandle handle,
              ...
              DSPLIB_cascadeBiquad_InitArgs *pKerInitArgs)
{
   ...
   {
      status = DSPLIB_cascadeBiquad_init_cn(handle,
                                                         bufParamsIn,
      ...
      if (bufParamsIn->data_type == DSPLIB_FLOAT32) {
       ...
       printf("DSPLIB_DEBUGPRINT bufParamsIn->data_type == DSPLIB_FLOAT32\n");
         ...
         if (pKerPrivArgs->initArgs.numStages == 3) {
            pKerPrivArgs->execute =
               DSPLIB_cascadeBiquad_exec_ci<float>;

            status = DSPLIB_cascadeBiquad_init_ci<
                  float>(
                      handle,
                      bufParamsIn,
                      bufParamsFilterCoeff,
           ...
           pKerPrivArgs->execute =
              DSPLIB_cascadeBiquad7Stage_exec_ci<
                 float>;
           status = DSPLIB_cascadeBiquad7Stage_init_ci<
                   float>(
                       handle,
                       bufParamsIn,
                       bufParamsFilterCoeff,
...
DSPLIB_STATUS DSPLIB_cascadeBiquad_exec(
   DSPLIB_kernelHandle handle,
   void *restrict pIn,
