...
int DSPLIB_add_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   add_testParams_t   *prm;
   add_testParams_t   currPrm;
   add_getTestParams(&prm, &test_cases);
	...
	if( pIn1 && pIn2 && pOut && pOutCn ) {
       ...
       printf("DSPLIB_DEBUGPRINT  DSPLIB_add_d CP 1 status_init %d\n", status_init);
	    ...
	    if (status_init == DSPLIB_SUCCESS) {
	       ...
	       status_init = DSPLIB_add_init(handle,
							                &bufParamsIn,
							                &bufParamsOut,
							                &kerInitArgs);
	       DSPLIB_asm(" MARK 1");
...
#endif
	    status_opt = DSPLIB_add_exec_checkParams(handle,
								                    pIn1,
								                    pIn2,
								                    pOut);
#if DSPLIB_DEBUGPRINT
   ...
   if (status_opt == DSPLIB_SUCCESS){
      ...
      DSPLIB_asm(" MARK 2");
      status_opt = DSPLIB_add_exec(handle,
                                      pIn1,
                                      pIn2,
                                      pOut);
      DSPLIB_asm(" MARK 3");
      TI_profile_stop();
       ...
       printf("DSPLIB_DEBUGPRINT  DSPLIB_add_d CP 5\n");
	    ...
	    DSPLIB_asm(" MARK 8");
	    status_nat = DSPLIB_add_exec(handle,
							 pIn1,
							 pIn2,
							 pOutCn);
	    DSPLIB_asm(" MARK 9");
...
#if DSPLIB_DEBUGPRINT
       printf("DSPLIB_DEBUGPRINT  DSPLIB_add_d CP 8 status_nat_vs_opt %d status_ref_vs_opt %d currentTestFail %d\n", status_nat_vs_opt, status_ref_vs_opt, currentTestFail);
