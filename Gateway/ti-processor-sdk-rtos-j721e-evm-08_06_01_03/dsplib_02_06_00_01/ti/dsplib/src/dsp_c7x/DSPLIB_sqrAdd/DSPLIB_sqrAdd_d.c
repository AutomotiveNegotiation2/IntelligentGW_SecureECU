...
int DSPLIB_sqrAdd_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   sqrAdd_testParams_t   *prm;
   sqrAdd_testParams_t   currPrm;
   sqrAdd_getTestParams(&prm, &test_cases);
	...
	if( pIn && pOut && pOutCn ) {
       ...
       printf("DSPLIB_DEBUGPRINT  DSPLIB_sqrAdd_d CP 1 status_init %d\n", status_init);
	    ...
	    if (status_init == DSPLIB_SUCCESS) {
	       ...
	       status_init = DSPLIB_sqrAdd_init(handle,
							                &bufParamsIn,
							                &bufParamsOut,
							                &kerInitArgs);
	       DSPLIB_asm(" MARK 1");
...
#endif
	    status_opt = DSPLIB_sqrAdd_exec_checkParams(handle,
								                    pIn,
								                    pOut);
#if DSPLIB_DEBUGPRINT
   ...
   if (status_opt == DSPLIB_SUCCESS){
      ...
      DSPLIB_asm(" MARK 2");
      status_opt = DSPLIB_sqrAdd_exec(handle,
                                      pIn,
                                      pOut);
      DSPLIB_asm(" MARK 3");
      TI_profile_stop();
       ...
       printf("DSPLIB_DEBUGPRINT  DSPLIB_sqrAdd_d CP 5\n");
	    ...
	    DSPLIB_asm(" MARK 8");
	    status_nat = DSPLIB_sqrAdd_exec(handle,
							 pIn,
							 pOutCn);
	    DSPLIB_asm(" MARK 9");
	    TI_profile_stop();
       ...
       comparisonDone = 1;
#if DSPLIB_DEBUGPRINT
       printf("DSPLIB_DEBUGPRINT  DSPLIB_sqrAdd_d CP 7 comparisonDone %d status_nat_vs_opt %d\n", comparisonDone, status_nat_vs_opt);
#endif
#else
...
#if DSPLIB_DEBUGPRINT
       printf("DSPLIB_DEBUGPRINT  DSPLIB_sqrAdd_d CP 8 status_nat_vs_opt %d status_ref_vs_opt %d currentTestFail %d\n", status_nat_vs_opt, status_ref_vs_opt, currentTestFail);
       printf("DSPLIB_DEBUGPRINT  DSPLIB_sqrAdd_d CP 8 status_init %d status_opt %d status_nat %d\n", status_init, status_opt, status_nat);
#endif
...
#else
	    // clear the counters between runs; normally handled by TI_profile_sqrAdd_test
...
#if defined(RTL_TEST)
	 // for RTL testing, request to return after sqrAddst fail
	 if(fail){
	    free(handle);
	    return fail;
	 }
...
/* Main call for insqrAddidual test projects */
