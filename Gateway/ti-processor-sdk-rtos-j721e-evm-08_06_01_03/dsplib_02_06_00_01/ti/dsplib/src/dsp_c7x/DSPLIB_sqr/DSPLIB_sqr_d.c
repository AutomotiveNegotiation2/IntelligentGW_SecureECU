...
int DSPLIB_sqr_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
	...
	if( pIn && pOut && pOutCn ) {
       ...
       printf("DSPLIB_DEBUGPRINT  DSPLIB_sqr_d CP 1 status_init %d\n", status_init);
	    ...
	    if (status_init == DSPLIB_SUCCESS) {
	       ...
	       status_init = DSPLIB_sqr_init(handle,
							                &bufParamsIn,
							                &bufParamsOut,
							                &kerInitArgs);
	       DSPLIB_asm(" MARK 1");
...
#endif
	    status_opt = DSPLIB_sqr_exec_checkParams(handle,
								                    pIn,
								                    pOut);
#if DSPLIB_DEBUGPRINT
   ...
   if (status_opt == DSPLIB_SUCCESS){
      ...
      DSPLIB_asm(" MARK 2");
      status_opt = DSPLIB_sqr_exec(handle,
                                      pIn,
                                      pOut);
      DSPLIB_asm(" MARK 3");
      TI_profile_stop();
       ...
       printf("DSPLIB_DEBUGPRINT  DSPLIB_sqr_d CP 5\n");
	    ...
	    DSPLIB_asm(" MARK 8");
	    status_nat = DSPLIB_sqr_exec(handle,
							 pIn,
							 pOutCn);
	    DSPLIB_asm(" MARK 9");
	    TI_profile_stop();
...
#if DSPLIB_DEBUGPRINT
       printf("DSPLIB_DEBUGPRINT  DSPLIB_sqr_d CP 8 status_nat_vs_opt %d status_ref_vs_opt %d currentTestFail %d\n", status_nat_vs_opt, status_ref_vs_opt, currentTestFail);
...
#if defined(RTL_TEST)
	 // for RTL testing, request to return after sqrst fail
	 if(fail){
	    free(handle);
	    return fail;
	 }
