...
ifeq ($(HOST_OS),$(filter $(HOST_OS),Windows_NT LINUX))
    ifneq ($(TARGET_CPU),)
        ifeq ($(TARGET_CPU),$(filter $(TARGET_CPU),C64T C64P C64 C66 C674 C67 C67P))
            HOST_COMPILER?=CGT6X
        else ifeq ($(TARGET_CPU),$(filter $(TARGET_CPU),C7100 RESERVED RESERVED RESERVED RESERVED))
            HOST_COMPILER?=CGT7X
...
ifeq ($(HOST_OS),Windows_NT)
    ...
    PATH_SEP=/
    PATH_SEPD=/
    ifeq ($(TARGET_SCPU),$(filter $(TARGET_SCPU),C7100 RESERVED RESERVED RESERVED RESERVED))
        HOST_COMPILER?=GPP
    else
        HOST_COMPILER?=GCC
    endif
