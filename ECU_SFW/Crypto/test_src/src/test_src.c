...
void Run_CyRng(uint8_t* output, uint32_t sz)
{
	// RNG FUNCTION
...
int test_aes128ctr_enc(uint8_t encdec_idx)
{
    ...
    size_t        length                = first_length + second_length; // 1024
...
uint8_t testing_parsing(void)
{
	...
	int32_t  fsz                   = 0;
	char* Sflash_sample = SAMPLE_MEMORY_ADDRESS;
...
//Header
		...
		memcpy(md_buf, Sflash_sample, fsz);
...
//2nd Block Memory allcoation
        ...
        dbg_printf("## META Reserved    [%d] = 0x%02X 0x%02X (FIX Value) \r\n", sizeof(header.reserve_use), header.reserve_use[0], header.reserve_use[1]);
        dbg_printf("----------------------  Header area End  -----------------------------\r\n");
        dbg_printf("\r\n\n");

		body.total_counter  = md_buf[16];
        dbg_printf("## Total Counter    [%d] = 0x%02X \r\n",sizeof(body.total_counter), body.total_counter);

// body start
/* ����� 1�� block������ 2�� block���� ���� �� ������ �ٰ� ������ ������.
   1. BLOCK_SIZE �� 10240 -> 5120 ���� ����.
   2. body.total_counter 1-> 2�� ����.(Bin File���� ����.)
   3. block count(1) + HMAC(32) + encrypt data(5120) �� �ݺ� 2ȸ ó�� �ؾ� ��.
   4. body start ~ body end �ݺ�.
   5. server ���� file �����ϴ� ��� Ȯ���� �ʿ�.
   6. HMAC�� ��� block ������ �����ϰ� siginature�� block ������ �ƴ� ��ü encrypt data(10240) ���� ���� �ʿ�.
*/
 ...
 * @details     main test function. \n
 *              int main(void) in this function
 * @param[in]	void
